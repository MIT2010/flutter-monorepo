# Verdant — Design System Specification v1.9
...
**v1.8 revision note**: two additions, both driven by a real downstream consumer rather than speculative rollout — a full-breadth template app (`apps/fintech_demo`, "Verdant Bank") built specifically to exercise every existing component surfaced two genuine gaps. `AppLineChart` (§10.14) fulfills the line-drawing shape §10.14's own Motion bullet already named but never had an implementation behind it — a portfolio/spending-trend screen needed a continuous-trend chart, not another discrete-category comparison `AppBarChart` already covers. `AppStepper` (§10.25) is a wholly new addition, outside the original seventeen-component brief — a multi-step KYC/registration flow needed a linear step-progress primitive with no existing Verdant answer (Tabs' peer-switching model doesn't fit an ordered, completable sequence). Both ship with the same bar as every other component here: golden-tested (light/dark), a Widgetbook use-case, `@verdantPreview` maturity (§15) pending real usage beyond this one consuming app.

**v1.9 revision note**: a direct visual-quality finding, not a component gap this time — asked to judge `apps/fintech_demo` purely on whether it looks genuinely polished rather than whether it's internally consistent, the honest answer was no. Every component individually followed its own §10 spec, but the Cards screen represented a bank card — the single most recognizable artifact in any banking app's UI — as a plain data row (a small network-badge tag plus two lines of text inside `AppCard`), which reads as a table, not a card. `AppPaymentCard` (§10.26) is the fix: a new component, not an app-local patch, following this document's own rule that a genuine design_system gap gets closed in the system. Solid-fill (`colorScheme.primary` by default, `colorScheme.tertiary` accepted to visually distinguish a second card in the same list — reusing existing roles rather than inventing a card-specific palette), standard ID-1 card proportions, no gradient (consistent with §5.4's anti-pattern ruling against `AppExpressiveCard`'s old color-block treatment), no shadow (§6 — a card at rest is page content, not a floating surface). `apps/fintech_demo`'s Home balance summary was given the same solid-fill treatment for the same reason — its hero number deserves real visual weight, not a hairline-bordered box indistinguishable from every other resting surface on the page.
...
**Implemented as of v1.8**: both shapes named above exist now — `AppBarChart` (bars rising) and `AppLineChart` (a line drawing left-to-right, with an optional flat single-series fill under it). They share `AppChartSeries` and the same five-color sequence rather than each defining their own.
...
**Added in v1.8** — not one of the seventeen §10.7–§10.23 components named in the original brief; added when a downstream consumer (a multi-step registration/KYC flow) needed a linear step-progress primitive design_system had no answer for. Same bar this document holds every other addition to: specified here, implemented alongside, golden-tested, not bolted on ad hoc in the consuming app.

- **Purpose**: shows *where you are* in a fixed, linear, multi-step flow (step 2 of 4) — distinct from Tabs (§10.10, which switches between peer views with no inherent order or completion state) and from a Dialog/Sheet's own internal pagination (which has no persistent chrome at all). A Stepper's steps are ordered and generally completed in sequence; a Tab's aren't.
- **Shape**: each step is a small circular marker (`radius.pill`, same "true pill/circle objects" family as Radio/Switch/Avatar, §5.3) connected by a straight `radius.none` line — no card, no container chrome around the whole strip.
- **Color**: a completed step fills `colorScheme.primary` with a checkmark (`VerdantIcon(VerdantGlyph.check)`, §16.2); the current step is an outlined `colorScheme.primary` ring around the step's own number; a future step is a flat `colorScheme.outlineVariant` circle with a muted number. The connecting line recolors to `colorScheme.primary` behind completed steps, `colorScheme.outlineVariant` ahead of them — the line itself communicates progress redundantly with the markers, not decoratively.
- **Depth**: flush, Level 0 — a stepper is chrome, not a floating surface.
- **Motion**: a step transitioning from current → completed animates its fill/checkmark in via `motion.micro` + Verdant Enter; the connecting line's color sweeps over the same duration rather than snapping — both respect `disableAnimations` (§8.7) by jumping to final state instead.
- **State**: purely presentational (`currentStep: int`, `stepCount: int`, `labels: List<String>?`) — no built-in tap-to-navigate affordance, since not every consuming flow allows jumping steps out of order; a consumer that wants that wraps each marker in its own `GestureDetector`/`InkWell` rather than this component assuming it's always safe.

### 10.26 Payment Card

**Added in v1.9** — a direct visual-quality finding, not a spec gap the original brief named. `apps/fintech_demo`'s Cards screen previously represented a bank card as a plain data row inside `AppCard`; a payment card is the single most recognizable artifact in any banking UI and deserves its own component, not a repurposed generic container.

- **Purpose**: display a bank/payment card as a real card visual — masked number, holder name, network label — distinct from `AppCard` (a generic content container) and from the small network-badge treatment `AppTable`/`AppList` rows use when a card needs to appear inline in a denser layout.
- **Shape**: standard ISO/IEC 7810 ID-1 proportions (1.586:1) via a fixed `AspectRatio`, `radius.sm` corners — the same tier `AppCard` uses (§10.2), keeping this consistent with §5's "precise, not soft" corner posture rather than a literal plastic card's more rounded corners.
- **Color**: solid fill, `colorScheme.primary` by default. **No gradient** — §5.4's `AppExpressiveCard` rewrite already retired gradient/color-block "expressive" treatments as an anti-pattern for this system; a solid brand-color fill gives the card real visual weight without reintroducing that. A caller may pass a different fill (e.g. `colorScheme.tertiary`) so a multi-card list reads as visually distinct entries rather than every card defaulting to the same tone — reusing existing `ColorScheme` roles, not a new card-specific palette.
- **Depth**: flush — no border, no shadow. The solid fill itself separates the card from the page; per §6, shadow is reserved for true floating overlays, and a payment card at rest is still page content.
- **Frozen state**: the fill desaturates toward `colorScheme.surfaceContainerHighest` (a theme-safe neutral, never a raw `VerdantColors` value from component code) and a small pill tag reading "Frozen" appears using `colorScheme.error`/`.onError` — the same danger role Cards page already reached for directly before this component existed.
- **Content**: a chip glyph (drawn locally, not added to `VerdantGlyph` — a one-off decorative shape this single component owns, not something the design system's own components render internally), the masked number in `letterSpacing: 2` for legibility, holder name (uppercased, left) and network label (right) on the bottom row.
...
## 16. Visual identity: the Verdant Corner

**Added in v1.6.** Where §15 answers "how proven is this component," this section answers a different question the v1.6 revision note raises: once every component is built, tested, and proven — does the *system as a whole* look like anything other than a quieter Material Design? A full visual audit answered no, and this section is what changed as a result. Applied across all 28 components (both `@verdantStable` and `@verdantPreview`) in one pass, not gated by maturity tier — this is a shape/icon/motion correction, not a new capability a consumer opts into.

### 16.1 The shape signature

Every rectangle-family container surface — buttons, cards, dialogs, sheets, menus, dropdown panels, tooltips, inputs, OTP cells, snackbars — has its **top-right corner chamfered** instead of rounded, at a size paired to that surface's own `radius.xs`/`.sm`/`.md` tier (§5). The other three corners keep the exact radius values §5 already specifies; the notch is additive, not a replacement scale.

**Deliberately not applied** to controls whose shape is a load-bearing affordance rather than decoration — `AppRadio` (a circle reads as "radio" independent of any design system), `AppSwitch`'s track/thumb (a pill+circle reads as "toggle" the same way), `AppAvatar` (a circle reads as "photo/identity" almost universally). Cutting a corner off any of these would cost immediate recognizability for no identity gain, since a user's shape-literacy for "this is a toggle" comes from decades of physical switches, not from any UI kit. §10.13's Table and §10.23's Menu items also don't take it at the row level — both already carry a documented, evidence-based reason their selection language skips the edge-bar/notch entirely (Table's own gridlines already provide structure; a menu item is a one-shot action, not a persisted state) and that reasoning isn't superseded here.

The cut corner's fill is `null` (transparent, showing whatever is behind the surface) by default — a purely structural cut. On **emphasis surfaces** — a primary button, a card mid-press, a focused input, a selected menu/dropdown row — it fills with a new semantic role, `AppSemanticColors.accent` (Brass). Deliberately not `warning` reused, even though both are Brass: a decorative brand accent and a warning signal sharing a hue is a coincidence of this palette having one warm secondary scale, not a claim that a notch means "something's wrong." Implemented as `VerdantNotchedBorder` (any `ShapeBorder` call site — `ElevatedButton.style.shape`, `Card.shape`, `DialogTheme.shape`) and `VerdantNotchedInputBorder` (`InputDecorationTheme`, a separate class hierarchy Flutter uses specifically for text fields) in `packages/design_system/lib/src/shape/verdant_notched_border.dart`.

### 16.2 The icon set

Every glyph `design_system` draws *internally* (§10.7's check/dash, §10.16/§10.18's search/eye/eye-off/close, §10.13's sort arrows, §10.19's month-nav chevrons, §10.22's dropdown chevron — eleven glyphs total) is now hand-drawn via a `CustomPainter`, not sourced from `Icons.*`. Two independent reasons converged on this, not one: a stock Material glyph is an instant, shape-independent "this is Material" tell no amount of color/corner work elsewhere removes; separately, a font-based glyph silently rasterizes as an empty box ("tofu") if the icon font isn't loaded in the `flutter_test` golden harness — the entire `@verdantPreview` rollout's golden PNGs had this happening in the background, unnoticed, because a hollow square still looks plausibly icon-shaped in a quick glance. A `Path` painted directly has no such failure mode. `VerdantIcons` (`packages/design_system/lib/src/icons/verdant_icons.dart`) is deliberately plain, confident strokes, not an attempt to make each 16–20px glyph individually distinctive — at that size an overworked glyph reads as broken, not crafted. Consumer-chosen icons (`AppSidebar`/`AppNavigationBar`/`AppMenuItem`'s `IconData` fields, `AppStateView.icon`) are unaffected — out of this package's scope, same as before.

### 16.3 Selection language, formalized

`AppSidebar` and `AppNavigationBar` already used an ad hoc left/top-edge accent bar for "this is selected" — correct per §8.2, just reimplemented independently each time. `VerdantEdgeAccent` (`packages/design_system/lib/src/widgets/accents/verdant_edge_accent.dart`) is that pattern promoted to one shared widget, now also used by `AppListRow` and `AppDropdown`'s option rows, so every row-style selection surface reaches for the same signal instead of six components each reinventing "fill it primary" — which was also, on its own, indistinguishable from how Material's own selected-state components work.

### 16.4 Motion: `curveEmphasis`

One additional curve on `AppMotionExtension`, reserved for the small set of affirmative moments the notch itself marks (a primary button's press-feedback). **Still zero overshoot** — §7's "never bouncy" rule is not relaxed here; an overshoot/spring treatment was considered and rejected specifically because it would contradict that rule. What reads as distinct is pace, not shape: nearly all deceleration happens in the first fraction of the curve, a crisper "snap to attention" than `curveEnter`'s gentle settle — the motion equivalent of a precise cut corner instead of a soft rounded one.
