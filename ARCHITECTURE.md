...
5. **Committing generated files** (`*.g.dart`, `*.freezed.dart`, `*.config.dart`, `*.module.dart`) inconsistently — pick one policy and apply it repo-wide via `.gitignore`, not per-package. This project's policy is now **don't commit them** (ADR-007) — `melos run gen` regenerates everything, and CI already runs it before analyze/test, so there's no build-step cost being avoided by committing. The one deliberate exception is hive_ce_generator's `*.g.yaml` schema-migration file, which stays tracked (its own generated header says to).
