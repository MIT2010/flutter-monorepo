...
- `AppExpressiveCard` — flagship shape+motion demo: morphs from a plain
  ...
  documented.
- `AppStateView` — token-driven placeholder for a screen's non-content
  states: empty data, labelled loading (`loading: true`, spinner replaces
  `icon`), or error+retry (`tone: AppStateViewTone.error` tints
  icon+message with `colorScheme.error`) — icon+message+optional-action
  skeleton, always centered. Renamed from `AppEmptyState` (icon was
  required, no `loading`/`tone`) once a component-gap audit found the same
  shape hand-rolled for loading/error states across this kit's own
  Home/Profile pages and 8+ screens in `akujamin-v2`, not just "empty"
  ones (design_system component-kit expansion audit, 2026-07-18 — see
  ADR-017). `NotFoundPage` (`packages/shared`) consumes it directly.
...
**ADR-017 — Design language brief adopted (Plus Jakarta Sans bundled locally); `AppEmptyState` renamed to `AppStateView` and extended with `loading`/`tone`; a component-gap audit grounded further `design_system` decisions in cited evidence rather than "modern apps usually have X."**
*Context (2026-07-18):* Two related efforts landed the same day, both explicitly evidence-gated (no code before an audited/reported proposal was confirmed). First, a **design language brief** (`docs/DESIGN_LANGUAGE.md`) named the character every component decision since should be checked against — "confident, warm, precise" — derived from decisions already shipped (the seed color's saturation, the brand weight overrides already in `AppTheme`'s `TextTheme`, the moderate radius scale, `AppExpressiveCard`'s deliberately non-bouncy spring tuning) plus the real domain the design system serves (`akujamin-v2`'s identity-verification/psychological-assessment/counseling/payment flows — trust and approachability both load-bearing, not just aesthetic preference). Typography was named the highest-impact/lowest-cost change: Plus Jakarta Sans (SIL OFL 1.1, variable `wght` 200-800, true per-weight italics) was chosen over Manrope (same license/axis, no italics) for its warmer character (rounded terminals) atop the same geometric precision. Second, a **component-gap audit** mapped repeated un-componentized UI patterns across this kit's own 4 real screens *and* `akujamin-v2`'s ~9 feature packages (evidence from the fork remains valid per ADR-016's finding that it's independently-evolving, not a live dependency), using the same 2+-genuine-occurrence bar `AppStatusBadge` was already held to — patterns with only 1 occurrence or occurring inside a single file were explicitly rejected, not componentized on a "modern apps have this" guess (chip/tag: 3 uses, 1 file, rejected; section headers: zero found; card/container ad-hoc: each occurrence served a different visual role, rejected; search/filter, rating/score, file-preview-beyond-`LocalImagePreview`: zero found).

*Decision — typography:* `google_fonts`' default runtime-CDN-fetch was tried first and rejected after it broke test determinism — any test constructing `AppTheme.light()`/`.dark()` (nearly the whole golden suite) started depending on network access to `fonts.gstatic.com`, reproduced as a real local test failure, not a hypothetical. Plus Jakarta Sans is instead **bundled as a local package asset** (`packages/design_system/fonts/`, two variable `.ttf` files — regular + italic — plus `OFL.txt`, downloaded from the font's canonical source, `github.com/tokotype/PlusJakartaSans`, with explicit user sign-off before any file was fetched) and applied via `_baseTextTheme.apply(fontFamily: 'Plus Jakarta Sans')` — a pure, synchronous, offline `TextTheme` transform, zero new runtime dependency (`google_fonts` was added then removed from `pubspec.yaml` once the bundled-asset path replaced it). One filename gotcha cost real debugging time and is worth recording: Flutter's asset bundler cannot resolve font filenames containing literal `[`/`]` characters (the font's upstream filenames, `PlusJakartaSans[wght].ttf`) — surfaced as `unable to locate asset entry in pubspec.yaml` even though the file demonstrably existed on disk; renamed to `PlusJakartaSans-VariableFont_wght.ttf` (no semantic content change, same bytes) and the error disappeared.

*Decision — `AppEmptyState` → `AppStateView`:* the audit found the exact icon+message+optional-action shape `AppEmptyState` already implemented was independently hand-rolled for loading and error+retry states too — not just "empty data" — 17× for a bare loading spinner, 8× near-identical for error+retry (Home/Profile in this kit; `about`/`history`/`certificate`/`counseling`-list/`chat`/`onboarding` in `akujamin-v2`, all using the same `AppButton(label: 'Coba lagi', ...)`), plus a 6-instance cross-cutting "icon-or-spinner + title + description + optional button" shape spanning register/test-result/onboarding screens in `akujamin-v2`. Rather than a new, structurally-duplicate `AppErrorState` widget, `AppEmptyState` was renamed to `AppStateView` and extended additively: `icon` became optional (was required — some error states in the wild have none), a `loading` bool shows a `CircularProgressIndicator` in place of the icon (closing the "labelled loading" sub-pattern), and a new `AppStateViewTone` enum (`neutral`/`error`) tints icon+message with `colorScheme.error` when set — matching the manual `colorScheme.error` styling Home/Profile's error text already got in an earlier same-repo change, now made reusable instead of bespoke per screen. All existing call sites (`NotFoundPage`) needed no changes beyond the rename — every new parameter defaults to the prior behavior. Golden coverage grew from 4 cases (with/without action × light/dark) to 8 (added loading and error-tone × light/dark).

*Deferred:* `AppAvatar` (circular image + `Icons.person` fallback, evidenced by 2 identical occurrences in `akujamin-v2`'s `home_page.dart`/`profile_page.dart`) meets the same evidence bar `AppStatusBadge` was held to, but has **zero current consumer in this kit** — `feature_profile`'s `Profile` entity has no photo field at all. Recorded here as a validated-but-not-built candidate rather than built ahead of any real need in this repo, consistent with this kit's own "don't build for hypothetical need" principle — build it if/when a feature here actually needs it, not preemptively because the pattern was proven elsewhere.

*Status:* Accepted. `docs/DESIGN_LANGUAGE.md` and the "Theme Studio" live-customization addon design (`apps/widgetbook/`) are the next-referenced artifacts for future component/token decisions in this section.
